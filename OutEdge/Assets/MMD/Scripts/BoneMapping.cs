...
namespace UniHumanoid
{
    public class BoneMapping : MonoBehaviour
    {
        ...
        public void EnsureTPose()
        {
            ...
            map[HumanBodyBones.Spine].position += new Vector3(0, -GetComponent<SkinnedMeshRenderer>().bounds.extents.y*0.8f, 0);

            map[HumanBodyBones.RightUpperLeg].localRotation = Quaternion.Euler(0f, 0f, -9f);
            map[HumanBodyBones.LeftUpperLeg].localRotation = Quaternion.Euler(0f, 0f, 2f); // Temporary

            map[HumanBodyBones.RightLowerLeg].localRotation = Quaternion.Euler(0f, 0f, 0f);
            map[HumanBodyBones.LeftLowerLeg].localRotation = Quaternion.Euler(0f, 10f, 0f);

            //EditorApplication.isPaused = true;
            //map[HumanBodyBones.RightLowerLeg].localRotation = Quaternion.Euler(0, 0, -20f);
        }

        public Avatar GetAvatar()
        {
            var description = AvatarDescription.Create(Description);
            BoneMapping.SetBonesToDescription(this, description);
            var avatar = description.CreateAvatarAndSetup(this.transform);
            return avatar;
        }
